...
async function depsForPackage (pack, ver, db, dev = false, restrictTo = null, modulesDir = null, depMap = null, depsExamined = []) {
  ...
  for (let [dep, req] of _.pairs(depList)) {
    ...
    try {
      ...
      if (!usedDepVer) {
        throw new Error("Could not find adequate version from package.json");
      }
    } catch (e) {
    ...
    if (!usedDepVer) {
      log.red(`Could not find the version used for ${dep}, so not getting ` +
              `its deps`);
      subDeps[dep] = {deps: {}, outdated: {}, subDeps: {}};
    } else {
