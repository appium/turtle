...
async function main () {
  ...
  if (_.size(outdated.outdated)) {
    log.red(`${proj}@${ver} has outdated dependencies:`);
    log.red(outdated.outdated);
  } else {
    log.green(`${proj}@${ver} has no outdated dependencies`);
  }
  if (_.size(outdated.subs)) {
    log.yellow(`${proj}@${ver} has submodules which have outdated dependencies:`);
